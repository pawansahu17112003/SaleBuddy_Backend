...
router.post('/insert_productVarients',upload.none(), function (req, res, next) {
    try {
...
router.post('/fetch_productvarient_by_product',function(req, res, next) {
    try{
        pool.query("select P.*,PV.* from products P,productvarients PV where P.productid=PV.productid and P.productid=?",[req.body.productid], function(error,result){
            if(error){
                console.log(error);
                res.status(200).json({status:false, message:"Database Error, Pls Contact Backend Team"})
            }

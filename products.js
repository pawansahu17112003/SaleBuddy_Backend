...
router.post('/insert_products',upload.single('productpicture'),function(req,res,next){
    try{
...
router.post('/fetch_product_by_brand',function(req, res, next) {
    try{
        pool.query("select P.*,B.* from products P,brands B where B.brandid=P.brandid and B.brandid=?",[req.body.brandid], function(error,result){
            if(error){
                console.log(error);
                res.status(200).json({status:false, message:"Database Error, Pls Contact Backend Team"})
            }

...
router.post('/userinterface_chk_address', function (req, res, next) {
    try {
        pool.query("select * from useraddress where (emailid=? or mobileno=?)", [req.body.mobileno, req.body.mobileno], function (error, result) {
            if (error) {
                res.status(200).json({ status: false, message: 'Database Error,Pls Contact Backend Team' })
            }
            else {
...
router.post('/userinterface_user_address_submit', function (req, res, next) {
    try {
        pool.query("insert into useraddress(emailid, mobileno, address, state, city, pincode, landmark, username, gender,nickname,area)values(?,?,?,?,?,?,?,?,?,?,?) ", [req.body.emailid, req.body.mobileno, req.body.address, req.body.state, req.body.city, req.body.pincode, req.body.landmark, req.body.username, req.body.gender, req.body.nickname, req.body.area], function (error, result) {
            if (error) {
                console.log(error)
                res.status(200).json({ status: false, message: 'Database Error,Pls Contact Backend Team' })
            }
...
router.post('/update_user', function (req, res, next) {
    try {
        pool.query("update useraddress set  emailid=?,mobileno=?, username=?, gender=? where addressid=? ", [req.body.emailid, req.body.mobileno, req.body.username, req.body.gender, req.body.addressid], function (error, result) {
            if (error) {
                console.log(error);
                res.status(200).json({ status: false, message: "Database Error, Pls Contact Backend Team" })
            }
